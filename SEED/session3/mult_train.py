...
class region_loss(nn.Module):
    def __init__(self, beta=0.1, lambda_reg=1.0, lambda_imp=1.0, lambda_max_alpha=1.0):        
        super(region_loss, self).__init__()  
        self.beta = beta
        self.lambda_reg = lambda_reg
        self.lambda_imp = lambda_imp
        self.lambda_max_alpha = lambda_max_alpha
    ...
    def total_loss(self, alpha):
        ...
        total = self.lambda_reg * reg_loss + self.lambda_imp * imp_loss + self.lambda_max_alpha * max_alpha_loss
...
def train_subject(num_subj=1,session_num=1,batch_size = 32,lr = 0.001,epochs = 240,T_max = 40, drop_rate = 0,):
    ...
    loss_function2 = region_loss(beta=0.1, lambda_reg=1.0, lambda_imp=1.0, lambda_max_alpha=1.0)
...
if __name__ == '__main__':
    ...
    for i in range(1, 16):
        ...
        with open(log_file, 'a') as file:  
            file.write(f'\n---------------------------------------------------------------------start subect:{i}---------------------------------------------------------------------\n ')
        best_model_acc, best_epoch = train_subject(num_subj=i, session_num=3, batch_size=batch_size, lr=lr, epochs=epoch, T_max=T_max, drop_rate=drop_rate,)
