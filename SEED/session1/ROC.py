...
def plot_roc(y_test, y_scores, classes=None):
    ...
    plt.plot(rf_FPR[0], rf_TPR[0], label=f'SA (area={rf_AUC[0]:.4f})', lw=2, linestyle='--',)
    plt.plot(rf_FPR[1], rf_TPR[1], label=f'NE (area={rf_AUC[1]:.4f})', lw=2, linestyle='--',)
    plt.plot(rf_FPR[2], rf_TPR[2], label=f'HA (area={rf_AUC[2]:.4f})', lw=2, linestyle='--',)
