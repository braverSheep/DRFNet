...
SEEDIV_data_dir = "../../../datasets/SEED/seed_4s/"
        ...
        if is_train:
            ...
            for trial in range(9):
                ...
                for t in range(X_de.shape[1]):
                    ...
                    self.sample_label_list.append(y+1)
                    # 测试集构建
        else:
            for trial in range(9,15):
                print(trial+1)
                X_de = scipy.io.loadmat(file_path)['de_LDS{}'.format(trial + 1)]
                # 首先进入对应session,然后再session里面获得对应标签
                y = int(labels[self.session_num-1][trial])
...
if __name__== "__main__":
    ...
    data_loader = getloader(SEEDIV_data_dir, subj_num=1, session_num=1, batch_size=32, is_train=False, is_drop_last=False, is_shuffle=True)
