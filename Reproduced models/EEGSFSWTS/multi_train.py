...
from SFSWTS_model import TransformerEncoder
#from model import *
# from DGCNN import *
...
def train_subject(num_subj=1,fold_num=1,batch_size = 32,lr = 0.001,epochs = 240,T_max = 40, drop_rate = 0,):
    ...
    net = TransformerEncoder(sentence_len=62, d_feature=384).to(device)
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(tra_loader):
            images, labels = data 
            _,cla_g,_ = net(images.to(device))            

            optimizer.zero_grad()

            loss = loss_function(cla_g, labels.to(device))
            loss.backward()
            optimizer.step()   
...
if __name__ == '__main__':
    ...
    # 超参数设置
    batch_size = 32           #每次训练输入模型的数据量 #0.0008
    lr = 0.00001              #0.00001 #0.006
    epoch = 50                #训练总轮数
