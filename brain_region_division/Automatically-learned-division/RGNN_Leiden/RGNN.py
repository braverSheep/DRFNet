...
class NewSGConv(SGConv):
    ...
    def norm(edge_index, num_nodes, edge_weight, improved=False, dtype=None):
        ...
        row, col = edge_index
        deg = scatter_add(torch.abs(edge_weight), row, dim=0, dim_size=num_nodes)#当edge_weight是负数，设置为0不是更好？
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        if not self.cached or self.cached_result is None:
            edge_index, norm = NewSGConv.norm(
                edge_index.to(torch.int64), x.size(0), edge_weight, dtype=x.dtype)

            for k in range(self.K):
                x = self.propagate(edge_index, x=x, norm=norm)
            self.cached_result = x

        return self.lin(self.cached_result)#self.lin = Linear(in_channels, out_channels, bias=bias)
