...
class Model2(nn.Module):
	def __init__(self, in_channels=5, num_class=4):
		...
		self.linear2 = torch.nn.ModuleList([ 
			nn.Sequential(nn.Linear((self.out_channels*16), self.fuse_dim),nn.BatchNorm1d(self.fuse_dim)),
			nn.Sequential(nn.Linear((self.out_channels*16), self.fuse_dim),nn.BatchNorm1d(self.fuse_dim)),
			nn.Sequential(nn.Linear((self.out_channels*16), self.fuse_dim),nn.BatchNorm1d(self.fuse_dim)),
			nn.Sequential(nn.Linear((self.out_channels*9), self.fuse_dim),nn.BatchNorm1d(self.fuse_dim)),
			nn.Sequential(nn.Linear((self.out_channels*4), self.fuse_dim),nn.BatchNorm1d(self.fuse_dim)),
			nn.Sequential(nn.Linear((self.out_channels*9), self.fuse_dim),nn.BatchNorm1d(self.fuse_dim)),
			nn.Sequential(nn.Linear((self.out_channels*9), self.fuse_dim),nn.BatchNorm1d(self.fuse_dim)),]
		)
