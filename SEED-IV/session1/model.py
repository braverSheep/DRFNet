...
class Model2(nn.Module):
	...
	def forward(self, data):
		...
		for i in range(self.branches):
			...
			x = self.cnn_tran_fuse[i](cnnx, tranx)
			# print(x.shape)
...
def count_and_convert_params(model):
	# 计算可学习参数量
	trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
	
	
	# bytes_per_param = 4
	# total_bytes = trainable_params * bytes_per_param
	total_mb = trainable_params / (1000 * 1000)  # 转换为MB

	return trainable_params, total_mb
...
if __name__=='__main__':
	...
	trainable_params, model_size_mb = count_and_convert_params(net)
