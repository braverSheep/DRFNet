...
def save_predictions(y_true, y_score, session_num):
    # 保存y_true和y_score
    torch.save({'y_true': y_true, 'y_score': y_score}, f'D:\\research\\文章\\multi_region\\SEEDIV_session{session_num}.pth')
...
if __name__ == '__main__':
    ...
    print(result_string)
    #保存结果
    save_predictions(y_true, y_score, session_num=3)
    ...
    conf_matrix = pd.DataFrame(cm, index=['NE','SA','FE','HA'], columns=['NE','SA','FE','HA'])  #数据有4个类别
    ...
    conf_matrix_custom_labels = ['NE','SA','FE','HA']
