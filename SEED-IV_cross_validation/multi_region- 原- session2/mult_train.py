...
def cre_prolog(cross_validation):
    # 获取当前脚本的绝对路径  
    current_script_path = os.path.abspath(__file__)  
    # 获取当前脚本的所在目录  
    parent_dir = os.path.dirname(current_script_path)  
    # 定义要创建的Pro_log目录的路径
    pro_log_dir_path = os.path.join(parent_dir, f'Pro_log{cross_validation}')  
...
def train_subject(num_subj=1,session_num=1,batch_size = 32,lr = 0.001,epochs = 240,T_max = 40, drop_rate = 0,cross_validation=0):

    tra_loader = getloader(data_dir, subj_num=num_subj, session_num=session_num, batch_size=batch_size, is_train=True, is_shuffle=True, cross_validation=cross_validation)
    tes_loader = getloader(data_dir, subj_num=num_subj, session_num=session_num, batch_size=batch_size, is_train=False, is_shuffle=True, cross_validation=cross_validation)
...
if __name__ == '__main__':
    ...
    for cross_validation in ([0, 1, 2, 3, 4]):
        exp_dir = cre_prolog(cross_validation)
        ...
        for i in range(1, 16):
            ...
            with open(log_file, 'a') as file:  
                file.write(f'\n---------------------------------------------------------------------start subect:{i}---------------------------------------------------------------------\n ')
            best_model_acc, best_epoch = train_subject(num_subj=i, session_num=2, batch_size=batch_size, lr=lr, epochs=epoch, T_max=T_max, drop_rate=drop_rate,cross_validation=cross_validation)
